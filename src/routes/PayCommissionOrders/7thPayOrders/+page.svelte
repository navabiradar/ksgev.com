<script>
  
</script>

<div class="container-xl">
    <div class="row row-card">
    <div class="col-md-12">
        <div class="card rounded-0 border-primary">
          <div class="card-body bg-blue rounded-0 text-center">
            <h2 class="text-uppercase text-white">7th Pay Orders</h2>
          </div>
          <div class="card-body">
            <table class="table table-responsive table-bordered border-primary text-center">
                <tbody>
                    <tr>
                    <td>
                    <p><strong>ಸರ್ಕಾರದ ಆದೇಶಸಂಖ್ಯೆ</strong></p>
                    </td>
                    <td><strong>ಆದೇಶದ ದಿನಾಂಕ</strong></td>
                    <td>
                    <p><strong>ವಿಷಯ</strong></p>
                    </td>
                    <td>
                    <p><strong>ಗಾತ್ರ</strong></p>
                    <p><strong>(</strong><strong>ಎಂ</strong><strong>.</strong><strong>ಬಿ</strong><strong>.</strong> <strong>ಗಳಲ್ಲಿ</strong><strong>)</strong></p>
                    </td>
                    <td>
                    <pre id="tw-target-text" class="tw-data-text tw-ta tw-text-medium" dir="ltr" data-fulltext="" data-placeholder="Translation"><strong><span lang="kn" tabindex="0"><span style="color: #000000;">ಡೌನ್ಲೋಡ್</span> </span></strong></pre>
                    </td>
                    </tr>
                    <tr>
                    <td>ಆಇ 21 ಎಸ್‌ ಆರ್‌ ಪಿ 2024 (I)</td>
                    <td>
                    <p>27-08-2024</p>
                    </td>
                    <td>
                    <p>ವೈದ್ಯಕೀಯ ಭತ್ಯೆ ದರಗಳ ಪರಿಷ್ಕರಣೆ</p>
                    </td>
                    <td>0.18</td>
                    <td><a href="https://finance.karnataka.gov.in/storage\pdf-files/FD21SRP2024(I)-Dtd-27-08-2024(Kan).pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    <tr>
                    <td>ಆಇ 21 ಎಸ್‌ ಆರ್‌ ಪಿ 2024</td>
                    <td>23-08-2024</td>
                    <td>
                    <p>ವೇತನ ಶ್ರೇಣಿಗಳ ಪರಿಷ್ಕರಣೆ ಮತ್ತು ಇತರೆ ಸಂಬಂಧಿತ ಆದೇಶಗಳು</p>
                    </td>
                    <td>12.6</td>
                    <td><a href="https://finance.karnataka.gov.in/storage\pdf-files/FD21SRP2024dt23-AUG-2024_K.pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    <tr>
                    <td>ಆಇ 21 ಎಸ್‌ ಆರ್‌ ಪಿ 2024</td>
                    <td>17-08-2024</td>
                    <td>
                    <p>ಕರ್ನಾಟಕ ನಾಗರೀಕ ಸೇವಾ (ಪರಿಷ್ಕೃತ ವೇತನ) ನಿಯಮಗಳು, 2024-ಅಧಿಸೂಚನೆ.</p>
                    </td>
                    <td>3.63</td>
                    <td><a href="https://finance.karnataka.gov.in/storage\pdf-files/FD21SRP2024-Dtd-17-08-2024(Kan).pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    <tr>
                    <td>ಆಇ 21 ಎಸ್‌ ಆರ್‌ ಪಿ 2024</td>
                    <td>22-07-2024</td>
                    <td>
                    <p>ಏಳನೇ ರಾಜ್ಯ ವೇತನ ಆಯೋಗದ ಶಿಫಾರಸ್ಸುಗಳನ್ನು ಅನುಷ್ಠಾನಗೊಳಿಸುವ ಕುರಿತು.</p>
                    </td>
                    <td>2.74</td>
                    <td><a href="https://finance.karnataka.gov.in/storage\pdf-files/FD21SRP2024-22-7-2024-KANcorrected.pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    <tr>
                    <td>ಆಇ 45 ಎಸ್.ಆರ್.ಪಿ 2022</td>
                    <td>
                    <p>06-11-2023</p>
                    </td>
                    <td>
                    <p>7ನೇ ರಾಜ್ಯ ವೇತನ ಆಯೋಗದ ಅವಧಿಯನ್ನು ವಿಸ್ತರಿಸುವ ಬಗ್ಗೆ.</p>
                    </td>
                    <td>1.75</td>
                    <td><a href="https://finance.karnataka.gov.in/storage\pdf-files/FD45SRP202-GovtOrderDtd-06-11-2023.pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    <tr>
                    <td>ಆಇ 45 ಎಸ್‌ ಆರ್‌ ಪಿ 2022</td>
                    <td>15-05-2023</td>
                    <td>
                    <p>7ನೇ ರಾಜ್ಯ ವೇತನ ಆಯೋಗದ ಅವಧಿಯನ್ನು ವಿಸ್ತರಿಸುವ ಬಗ್ಗೆ.</p>
                    </td>
                    <td>0.60</td>
                    <td><a href="https://finance.karnataka.gov.in/storage\pdf-files/FD45SRP2022-Dtd-15-05-2023.pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    <tr>
                    <td>ಆಇ 45 ಎಸ್ ಆರ್‌ ಪಿ 2022</td>
                    <td>08-03-2023</td>
                    <td>
                    <p>7ನೇ ರಾಜ್ಯ ವೇತನ ಆಯೋಗದ ಪರಿಶೀಲನಾರ್ಹ ಅಂಶಗಳಿಗೆ ಸೇರ್ಪಡೆ ಮಾಡುವ ಕುರಿತು.</p>
                    </td>
                    <td>0.74</td>
                    <td><a href="https://finance.karnataka.gov.in/storage\pdf-files/FD45SRP2022-Dtd-08-03-2023.pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    <tr>
                    <td>ಆಇ 07 ಎಸ್‌ ಆರ್‌ ಪಿ 2023</td>
                    <td>
                    <p>01-03-2023</p>
                    </td>
                    <td>
                    <p>ರಾಜ್ಯ ಸರ್ಕಾರಿ ನೌಕರರಿಗೆ ತಾತ್ಕಾಲಿಕ ಪರಿಹಾರ ಮಂಜೂರಾತಿ</p>
                    </td>
                    <td>0.22</td>
                    <td><a href="https://finance.karnataka.gov.in/storage\pdf-files/FD07SRP2023-Dtd-01-03-2023.pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    <tr>
                    <td>ಆಇ 45 ಎಸ್ ಆರ್ ಪಿ 2022</td>
                    <td>06-02-2023</td>
                    <td>
                    <p>7ನೇ ರಾಜ್ಯ ವೇತನ ಆಯೋಗದ ಪ್ರಶ್ನಾವಳಿಗಳಿಗೆ ಮಾಹಿತಿ ಒದಗಿಸುವ ಕುರಿತು – ಸುತ್ತೋಲೆ</p>
                    </td>
                    <td>2.07</td>
                    <td><a href="https://finance.karnataka.gov.in/storage/pdf-files/FD 45 SRP 2022 Dt06-02-2023.pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    <tr>
                    <td>ಆಇ 46 ಎಸ್‌ ಆರ್‌ ಪಿ 2022</td>
                    <td>01-12-2022</td>
                    <td>
                    <p>7ನೇ ರಾಜ್ಯ ವೇತನ ಆಯೋಗಕ್ಕೆ ಹುದ್ದೆಗಳನ್ನು ಸೃಜಿಸುವ ಬಗ್ಗೆ.</p>
                    </td>
                    <td>1.93</td>
                    <td><a href="https://finance.karnataka.gov.in/storage/pdf-files/FD46SRP2022-Dtd-1-12-2022.pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    <tr>
                    <td>ಆಇ 45 ಎಸ್‌ ಆರ್‌ ಪಿ 2022</td>
                    <td>
                    <p>19-11-2022</p>
                    </td>
                    <td>
                    <p>7ನೇ ರಾಜ್ಯ ವೇತನ ಆಯೋಗವನ್ನು ರಚಿಸುವ ಬಗ್ಗೆ.</p>
                    </td>
                    <td>0.20</td>
                    <td><a href="https://finance.karnataka.gov.in/storage/pdf-files/FD45SRP2022-Dtd-19-11-2022-Kan.pdf" target="_blank" rel="noopener noreferrer">ವೀಕ್ಷಿಸಿ</a></td>
                    </tr>
                    </tbody>
                </table>
          </div>
        </div>
    </div>
   
    </div>
  </div>